...
<section>
    ...
    <div class="contacts">
        <c:forEach var="type" items="${resume.contacts.keySet()}">
            <jsp:useBean id="type" type="ru.javawebinar.webapp.model.ContactType"/>
                <div class="contact"><%=HtmlUtil.getContact(resume, type)%></div>
        </c:forEach>
    </div>
    ...
    <div class="sections">
        <c:forEach var="entry" items="${resume.sections.entrySet()}">
            ...
            %>
            <div class="section">
                <div class="sectionname"><a name="<%=sectionType.name()%>"><%=sectionType.getTitle()%></a></div>
                <div class="sectioncontent"><%=section.toHtml()%></div>
            </div>
        </c:forEach>
    </div>
    </div>
